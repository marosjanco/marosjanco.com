...
Does anyone run this at serious scale? LangGraph's own docs cite Klarna, Uber, and J.P. Morgan. But the interesting part isn't the logos — it's that teams building production agents keep converging on the same shape: deterministic routing (the business knows its workflows, so the model doesn't freelance on process), escalation rules encoded as conditional edges a compliance reviewer can read, checkpointing for anything long-running, human sign-off as interrupts before consequential actions, and traces on everything.
...
1. **Build the document analyst** above. Feed it a photo of any handwritten note — gym schedule, groceries — and ask for extraction, a calculation, and a summary.
...
This essay is based on Episode 6 of the HuggingFace Agents Course Podcast. **[Listen on Spotify](https://open.spotify.com/show/033IY4yRcIIausR9EybtAm)**
...
**Shipped an agent that survived a crash?** Share the story. Tweet [@maros_janco](https://twitter.com/maros_janco) or email maros@marosjanco.com. I want to see what you're building.
